...
## Try for no symmetry
        ...
        if not two_D_flag:
            ...
            for j in range(2,cut+1):
                for t in range(0,slices):
                    output.write(array[len(array)-j*slices+t]+"\n")
    ...
    def symmetry_xyz():
        ...
        if not two_D_flag:
            ...
            for j in range(2,cut+1):
                for t in range(0,slices):
                    output.write(array_2[i-1-j*slices+t]+"\n")
        ...
        if item.endswith(".txt") ==True:
            if "grid" in item:
                ...
                if gridSpecs[0]!='GRID':
                    ...
                    slices=int(float(gridSpecs[2])) ## have to make it float first
                    step=float(gridSpecs[3])
                    symmetry=gridSpecs[7]
                    two_D_flag=True
