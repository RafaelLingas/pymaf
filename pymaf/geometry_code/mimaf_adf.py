...
class runs:
    ...
    def dalton_run(self):
        ...
        ###   GHOST GEOM  ######put the ghost in specific sized lists to then be used
        ghost_geom=[]
        ...
        if self.dim=="2d":
            if self.plane=='xy':
                ...
                for x in range(0,self.size-self.x_sym):
                        for y in range(0, self.size-self.y_sym-side):
                            ghost_geom.append("X "+self.pos[y, z, x] + " " + 
        ...
        for item in self.atoms:
            if item.id not in atom_types:
                atom_types.append(item.id)

        ## find atom types and add 1 for ghost ##
        base_file.append("AtomTypes="+str(len(atom_types)+1)+" Charge="+self.charge+" NoSymmetry Angstrom\n")
        ...
        for item in ghost_geom:
            ...
            if new_file_flag:
                ...
                if file_number>99:
                    file_prefix=''
                output=open(self.name+"_"+file_prefix+str(file_number)+".mol",'w+')
                self.text_log.append("Created "+str(file_number)+" files")
