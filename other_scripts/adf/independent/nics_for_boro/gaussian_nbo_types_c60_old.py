...
def file_search(fname):
    ...
    # lv=Orbital("lv")
    type_1=Orbital("host")
    type_2=Orbital("guest")
    all=Orbital("all")


    types=(core,sigma,pi,type_1,type_2,all,total)#,type_1,type_2)#,ry_p1,ry_p2,ry_d1,ry_d2)
    ...
    with open(data_file,'r') as input_data:
        ...
        for line in input_data:
            if check_orbs_flag==True:
                if found_orbs_flag==True and 'NHO DIRECTIONALITY' in line:#('RY' in line or 'NHO DIRECTIONALITY' in line):
                    n_lewis=int(uncut_BD[0][:-1])
                    check_orbs_flag=False
                    found_orbs_flag=False
                    finding_orbs_flag=False
                    # print(len(sigma.orbs))
                ...
                if found_orbs_flag ==True:
                    if 'CR' in line:
                        ...
                        aa=uncut_ncores[len(uncut_ncores)-2]
                        ...
                        if aa <= 60:
                            type_2.orbs.append(n_cores)
                        else:
                            type_1.orbs.append(n_cores)
                    ...
                    if 'RY' in line:
                        ...
                        match_C= re.search(r'\) C(.*?)s\(',line)
                        # match_H= re.search(r'\) C(.*?)s\(',line)

                        if match_C:
                            aa=int(match_C.group(1))
